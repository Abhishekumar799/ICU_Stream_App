...
model = load_model('Data/mortality_prediction_finetune_xgb_precision')
...
def get_data():
    data = pd.read_csv("Data/peerj-08-10337-s002.csv")
    data.columns = list(map(str.strip, list(data.columns)))
    data = data[['Age', 'Gender', 'Fever', 'Cough', 'SOB', 'Fatigue', 'Sputum', 'Myalgia', 'Diarrhea',
             'Nausea.or.vomiting', 'Sore.throat', 'Chest.discomfort..Chest.pain', 'smoking_history',
             'hypertensionhx', 'diabeteshx', 'coronaryheartdiseasehx', 'copdhx', 'carcinomahx', 'ckdhx', 'ALT',
            'HR', 'Lymphocytes', 'SpO2', 'Procalcitonin', 'RR', 'Systolic.BP', 'Temperature', 'death' ]]
    return data


def main():
    data = get_data()
    image2 = Image.open('Images/mortality.png')
    st.sidebar.info('This app is created to predict a particular patient might die within 6hrs or not [DeepAarogya]] - Version 1')
    st.sidebar.image(image2)
    st.title("Mortality Prediction V1")
    ...
    if check_data:
        ...
        db = Image.open('Images/mortality_ft_importance.png')
        st.image(db)

    test_model = st.sidebar.checkbox('Test Model V1 Online', True)
    if test_model:
        cols = data.columns
        print(cols)
        ...
        if st.checkbox("Do you have patient Lymphocyte?", False):
            Lymphocyte = st.number_input('Lymphocyte:', min_value=data.describe()["Lymphocytes"].loc["min"],
                                        max_value=data.describe()["Lymphocytes"].loc["max"],
                                        value=data.describe()["Lymphocytes"].loc["50%"])
        else:
            Lymphocyte = np.nan
        ...
        input_dict = {
            ...
            'Nausea.or.vomiting': cat_map[Nausea],
        ...
        if st.button("Predict"):
            ...
            print(output)
            if output == 1:
                st.warning(f"⚠️ Patient may die !!! (Confidence = {confidence*100} %)", )
            else:
                st.success(f"✅ Patient is fine right now !!! (Confidence = {confidence*100} %)")
